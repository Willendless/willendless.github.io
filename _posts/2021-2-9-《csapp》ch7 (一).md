...
最终，在命令行中运行程序时，shell会调用操作系统提供的`loader`程序，它会把可执行文件*prog*的数据和代码拷贝到内存，同时将控制转移到程序的开头。
...
![object_file.png](https://i.loli.net/2021/02/09/Ihkubr8iMW7VHXF.png)
...
+ *.rel.text:* 该节包含了`.text`节中的一系列待修改的位置，这些位置需要linker把各个目标文件链接在一起后修改。总的来说，任何调用外部函数或引用了外部变量的指令都需要修改。调用本地函数的指令一般无需修改。注意到在可执行目标文件中，该信息是不需要的。但是该信息一般都会保留，除非用户指明linker不包含它。
  + 调用外部函数的指令
  + 引用外部变量的指令
+ *.rel.data:* 该节包含了本模块定义或引用的全局变量的重定位信息。总的来说，外部定义的函数以及对外部变量的引用的重定位信息需要被修改。
...
+ `section`域：每个符号都对应目标文件的某个节，`section`域给出符号对应节在section header表中的索引。有三种伪节(pseudosections)在section header表中不存在条目。且这些伪节符号仅存在于可重定位目标文件，可执行目标文件中不存在。
  ...
  + *COMMON*: 未初始化的符号（还没有分配内存空间），对这类符号
    + *value*域给出对齐要求
    + *size*域给出最小的大小

`COMMON`和`.bss`节的区别在于

+ `COMMON`节给出：未初始化的全局变量
...
在编译期，编译器会将每个全局符号导出给汇编器并表明是*strong*或是*weak*符号。之后，汇编器会将这个信息隐含地编码在可重定位目标文件的符号表中。
...
在x86-64/Linux机器上，double占用8字节，int占用4字节。然而目标文件中x地址为`0x601020`，y地址为`0x601024`。当执行`x = -0.0`时，对x的赋值会覆写y的内存。

使用GCC时可以用`-fno-common`选项，当存在同名全局符号时会直接报错。或者使用`-Werror`选项，将所有警告都转换成error。

之前提到全局符号可能被分配到`COMMON`伪节或是`.bss`节，其区别就在于是否是强符号。若初始化为0或是静态变量则是强符号且在`.bss`节，否则则在`COMMON`节。
...
linker在符号决议完成之后，能够确定代码、数据节的具体大小，之后的重定位过程会将输入目标模块合并同时赋给每个符号运行时地址。重定位的步骤如下

1. 重定位节和符号定义：linker将所有相同类型的节合并为可执行目标文件的单个节。之后linker给每一个输入模块的节和定义的符号赋予运行时地址。**该步骤之后，每一条指令和程序中的全局变量都有了独一无二的运行时内存地址。**
...
重定位算法如下，其中的`ADDR(s)`表示linker为节选择的运行时地址，`ADDR(r.symbol)`表示为符号选择的运行时地址。第三行的`refptr`给出了引用在节中的地址。
...
当loader运行时，会根据program header table将可执行目标文件拷贝到内存。之后loader会跳转到程序entry point，这总是`_start`函数的地址。该函数在系统目标文件`crt1.o`中定义。`_start`函数会调用*system startup function*（即`libc.so`中定义的`__libc_start_main`函数）。它会初始化执行环境、调用用户态的`main`函数、处理返回值，并在必要时返回控制给内核。

## 10. 小结

+ 编译期、汇编器已经处理的问题：生成重定位条目。
+ 符号决议的意义：将对全局符号的引用绑定到唯一的定义上。处理不同目标文件中符号冲突。
